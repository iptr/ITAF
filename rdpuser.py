    ...
    def read(data):
        ...
        for l in datas:
            ...
            if len(l_strip) == 0:
                if len(packet) == 0:
                    continue
                ret.append(Packet(last_direction, ''.join(packet)))
                packet = []
                continue
            direction = l[0] != ' '
            # Check direction.
...
class VirtualConnector:
    ...
    def __init__(self, userIP, userPort, wtaIp, wtaPort):
        ...
        self.serversocket.bind(('', userPort))
    ...
    def connect(self):
        ...
        try:
            ...
            print('Connecting to', self.wtaIp, self.wtaPort)
            c_sock.connect((self.wtaIp, self.wtaPort))
            (a_sock, address) = self.serversocket.accept()
            return (c_sock, a_sock)
...
class Worker(threading.Thread):
    ...
    def test(self):
        ...
        try:
            ...
            (c_sock, a_sock) = self.connector.connect()
            print("Connected %d." % self.num)

            if self.timeout:
                c_sock.settimeout(self.timeout)
                a_sock.settimeout(self.timeout)
            ...
            while pos < len(self.packets):
                ...
                while pos_end < len(self.packets):
                    ...
                    pos_end += 1
                try:
                    self.send_packet(pos, pos_end, c_sock, a_sock)
    ...
    def send_packet(self, pos, pos_end, c_sock, s_sock):
        '''
